...
export class Server extends IPCServer {
	...
	private static getOnDidClientConnect(): Event<ClientConnectionEvent> {
		const onHello = Event.fromNodeEventEmitter<WebContents>(ipcMain, 'biggain:hello', ({ sender }) => sender);
		...
		return Event.map(onHello, webContents => {
			...
			const onMessage = createScopedOnMessageEvent(id, 'biggain:message') as Event<VSBuffer>;
			const onDidClientDisconnect = Event.any(Event.signal(createScopedOnMessageEvent(id, 'biggain:disconnect')), onDidClientReconnect.event);
