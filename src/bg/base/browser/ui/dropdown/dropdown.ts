...
import { Gesture, EventType as GestureEventType } from 'bg/base/browser/touch';
import { ActionRunner, IAction } from 'bg/base/common/actions';
import { IDisposable } from 'bg/base/common/lifecycle';
import { IContextViewProvider, IAnchor, AnchorAlignment } from 'bg/base/browser/ui/contextview/contextview';
import { IMenuOptions } from 'bg/base/browser/ui/menu/menu';
import { KeyCode } from 'bg/base/common/keyCodes';
import { EventHelper, EventType, append, $, addDisposableListener, DOMEvent } from 'bg/base/browser/dom';
		...
		for (const event of [EventType.MOUSE_DOWN, GestureEventType.Tap]) {
			this._register(addDisposableListener(this._label, event, e => {
				if (e instanceof MouseEvent && e.detail > 1) {
					return; // prevent multiple clicks to open multiple context menus
				}
