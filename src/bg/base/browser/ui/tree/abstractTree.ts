...
import { IDisposable, dispose, Disposable, toDisposable, DisposableStore } from 'bg/base/common/lifecycle';
import { IListOptions, List, IListStyles, MouseController, DefaultKeyboardNavigationDelegate, isInputElement, isMonacoEditor } from 'bg/base/browser/ui/list/listWidget';
import { IListVirtualDelegate, IListRenderer, IListMouseEvent, IListContextMenuEvent, IListDragAndDrop, IListDragOverReaction, IKeyboardNavigationLabelProvider, IIdentityProvider, IKeyboardNavigationDelegate } from 'bg/base/browser/ui/list/list';
import { append, $, getDomNodePagePosition, hasParentWithClass, createStyleSheet, clearNode, addDisposableListener } from 'bg/base/browser/dom';
import { Event, Relay, Emitter, EventBufferer } from 'bg/base/common/event';
import { StandardKeyboardEvent } from 'bg/base/browser/keyboardEvent';
import { KeyCode } from 'bg/base/common/keyCodes';
import { ITreeModel, ITreeNode, ITreeRenderer, ITreeEvent, ITreeMouseEvent, ITreeContextMenuEvent, ITreeFilter, ITreeNavigator, ICollapseStateChangeEvent, ITreeDragAndDrop, TreeDragOverBubble, TreeVisibility, TreeFilterResult, ITreeModelSpliceEvent, TreeMouseEventTarget } from 'bg/base/browser/ui/tree/tree';
import { ISpliceable } from 'bg/base/common/sequence';
import { IDragAndDropData, StaticDND, DragAndDropData } from 'bg/base/browser/dnd';
import { range, equals, distinctES6, firstOrDefault } from 'bg/base/common/arrays';
import { ElementsDragAndDropData } from 'bg/base/browser/ui/list/listView';
import { DomEmitter } from 'bg/base/browser/event';
import { fuzzyScore, FuzzyScore } from 'bg/base/common/filters';
import { getVisibleState, isFilterResult } from 'bg/base/browser/ui/tree/indexTreeModel';
import { disposableTimeout } from 'bg/base/common/async';
import { isMacintosh } from 'bg/base/common/platform';
import { clamp } from 'bg/base/common/numbers';
import { ScrollEvent } from 'bg/base/common/scrollable';
import { SetMap } from 'bg/base/common/collections';
import { treeItemExpandedIcon, treeFilterOnTypeOnIcon, treeFilterOnTypeOffIcon, treeFilterClearIcon } from 'bg/base/browser/ui/tree/treeIcons';
...
class TypeFilterController<T, TFilterData> implements IDisposable {
	...
	private updateFilterOnTypeTitleAndIcon(): void {
		if (this.filterOnType) {
			...
			this.filterOnTypeDomNode.title = "Disable Filter on Type";
		} else {
			...
			this.filterOnTypeDomNode.title = "Enable Filter on Type";
		}
	}
	...
	private render(): void {
		...
		if (this.pattern && this.tree.options.filterOnType && noMatches) {
			this.messageDomNode.textContent = "No elements found";
			this._empty = true;
		} else {
		...
		this.domNode.title = `Matched ${this.filter.matchCount} out of ${this.filter.totalCount} elements`;
