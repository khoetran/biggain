...
import * as path from 'bg/base/common/path';
import * as pfs from 'bg/base/node/pfs';
import * as cp from 'child_process';
import * as process from 'bg/base/common/process';
import * as Types from 'bg/base/common/types';
import { IStringDictionary } from 'bg/base/common/collections';
import * as Objects from 'bg/base/common/objects';
import * as extpath from 'bg/base/common/extpath';
import * as Platform from 'bg/base/common/platform';
import { LineDecoder } from 'bg/base/node/decoder';
import { CommandOptions, ForkOptions, SuccessData, Source, TerminateResponse, TerminateResponseCode, Executable } from 'bg/base/common/processes';
...
function terminateProcess(process: cp.ChildProcess, cwd?: string): Promise<TerminateResponse> {
	if (Platform.isWindows) {
		...
		try {
			const cmd = FileAccess.asFileUri('bg/base/node/terminateProcess.sh', require).fsPath;
			return new Promise(resolve => {
...
export abstract class AbstractProcess<TProgressData> {
	...
	public start(pp: ProgressCallback<TProgressData>): Promise<SuccessData> {
		if (Platform.isWindows && ((this.options && this.options.cwd && extpath.isUNC(this.options.cwd)) || !this.options && extpath.isUNC(process.cwd()))) {
			return Promise.reject(new Error('Can\'t execute a shell command on a UNC drive.'));
		}
