...
import { ICodeWindow, LoadReason, UnloadReason } from 'bg/platform/windows/electron-main/windows';
import { handleVetos } from 'bg/platform/lifecycle/common/lifecycle';
import { isMacintosh, isWindows } from 'bg/base/common/platform';
import { Disposable, DisposableStore } from 'bg/base/common/lifecycle';
import { Promises, Barrier, timeout } from 'bg/base/common/async';
import { NativeParsedArgs } from 'bg/platform/environment/common/argv';
import { assertIsDefined } from 'bg/base/common/types';
import { cwd } from 'bg/base/common/process';
...
export class LifecycleMainService extends Disposable implements ILifecycleMainService {
	...
	private onBeforeUnloadWindowInRenderer(window: ICodeWindow, reason: UnloadReason): Promise<boolean /* veto */> {
		return new Promise<boolean>(resolve => {
			...
			const okChannel = `biggain:ok${oneTimeEventToken}`;
			const cancelChannel = `biggain:cancel${oneTimeEventToken}`;
			...
			window.send('biggain:onBeforeUnload', { okChannel, cancelChannel, reason });
		});
	}
	...
	private onWillUnloadWindowInRenderer(window: ICodeWindow, reason: UnloadReason): Promise<void> {
		return new Promise<void>(resolve => {
			const oneTimeEventToken = this.oneTimeListenerTokenGenerator++;
			const replyChannel = `biggain:reply${oneTimeEventToken}`;

			ipcMain.once(replyChannel, () => resolve());

			window.send('biggain:onWillUnload', { replyChannel, reason });
		});
	}
