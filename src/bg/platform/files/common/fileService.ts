...
import { Disposable, IDisposable, toDisposable, dispose, DisposableStore } from 'bg/base/common/lifecycle';
import { IFileService, IResolveFileOptions, FileChangesEvent, FileOperationEvent, IFileSystemProviderRegistrationEvent, IFileSystemProvider, IFileStat, IResolveFileResult, ICreateFileOptions, IFileSystemProviderActivationEvent, FileOperationError, FileOperationResult, FileOperation, FileSystemProviderCapabilities, FileType, toFileSystemProviderErrorCode, FileSystemProviderErrorCode, IStat, IFileStatWithMetadata, IResolveMetadataFileOptions, etag, hasReadWriteCapability, hasFileFolderCopyCapability, hasOpenReadWriteCloseCapability, toFileOperationResult, IFileSystemProviderWithOpenReadWriteCloseCapability, IFileSystemProviderWithFileReadWriteCapability, IResolveFileResultWithMetadata, IWatchOptions, IWriteFileOptions, IReadFileOptions, IFileStreamContent, IFileContent, ETAG_DISABLED, hasFileReadStreamCapability, IFileSystemProviderWithFileReadStreamCapability, ensureFileSystemProviderError, IFileSystemProviderCapabilitiesChangeEvent, IReadFileStreamOptions, FileDeleteOptions, FilePermission, NotModifiedSinceFileOperationError, IFileChange, IRawFileChangesEvent } from 'bg/platform/files/common/files';
import { URI } from 'bg/base/common/uri';
import { Emitter } from 'bg/base/common/event';
import { IExtUri, extUri, extUriIgnorePathCase, isAbsolutePath } from 'bg/base/common/resources';
import { TernarySearchTree } from 'bg/base/common/map';
import { isNonEmptyArray, coalesce } from 'bg/base/common/arrays';
import { ILogService } from 'bg/platform/log/common/log';
import { VSBuffer, VSBufferReadable, readableToBuffer, bufferToReadable, streamToBuffer, VSBufferReadableStream, VSBufferReadableBufferedStream, bufferedStreamToBuffer, newWriteableBufferStream } from 'bg/base/common/buffer';
import { isReadableStream, transform, peekReadable, peekStream, isReadableBufferedStream, newWriteableStream, listenStream, consumeStream } from 'bg/base/common/stream';
import { Promises, ResourceQueue, ThrottledWorker } from 'bg/base/common/async';
import { CancellationTokenSource, CancellationToken } from 'bg/base/common/cancellation';
import { Schemas } from 'bg/base/common/network';
import { readFileIntoStream } from 'bg/platform/files/common/io';
...
export class FileService extends Disposable implements IFileService {
	...
	private async validateWriteFile(provider: IFileSystemProvider, resource: URI, options?: IWriteFileOptions): Promise<IStat | undefined> {
		...
		if (unlock && !(provider.capabilities & FileSystemProviderCapabilities.FileWriteUnlock)) {
			throw new Error(`Unable to unlock file '${this.resourceForError(resource)}' because provider does not support it.`);
		}
	...
	private async doReadFileStream(provider: IFileSystemProviderWithFileReadWriteCapability | IFileSystemProviderWithOpenReadWriteCloseCapability | IFileSystemProviderWithFileReadStreamCapability, resource: URI, options?: IReadFileStreamOptions & { preferUnbuffered?: boolean; }): Promise<IFileStreamContent> {
		...
		try {
			...
			const message = `Unable to read file '${this.resourceForError(resource)}' (${ensureFileSystemProviderError(error).toString()})`;
	...
	private readFileBuffered(provider: IFileSystemProviderWithOpenReadWriteCloseCapability, resource: URI, token: CancellationToken, options: IReadFileStreamOptions = Object.create(null)): VSBufferReadableStream {
		...
		readFileIntoStream(provider, resource, stream, data => data, {
			...
			errorTransformer: error => new FileOperationError(`Unable to read file '${this.resourceForError(resource)}' (${ensureFileSystemProviderError(error).toString()})`, toFileOperationResult(error), options)
		}, token);
