...
import { IFileSystemProviderWithFileReadWriteCapability, FileSystemProviderCapabilities, IFileChange, IWatchOptions, IStat, FileOverwriteOptions, FileType, FileDeleteOptions, FileWriteOptions, FileChangeType, createFileSystemProviderError, FileSystemProviderErrorCode } from 'bg/platform/files/common/files';
...
const ERR_FILE_IS_DIR = createFileSystemProviderError("File is Directory", FileSystemProviderErrorCode.FileIsADirectory);
const ERR_FILE_NOT_DIR = createFileSystemProviderError("File is not a directory", FileSystemProviderErrorCode.FileNotADirectory);
const ERR_DIR_NOT_EMPTY = createFileSystemProviderError("Directory is not empty", FileSystemProviderErrorCode.Unknown);

// Arbitrary Internal Errors (should never be thrown in production)
const ERR_UNKNOWN_INTERNAL = (message: string) => createFileSystemProviderError(`Internal error occurred in IndexedDB File System Provider. ($message{0})`, FileSystemProviderErrorCode.Unknown);
...
export class IndexedDB {
	...
	constructor() {
		this.indexedDBPromise = this.openIndexedDB(INDEXEDDB_BIGGAIN_DB, 2, [INDEXEDDB_USERDATA_OBJECT_STORE, INDEXEDDB_LOGS_OBJECT_STORE]);
	}
