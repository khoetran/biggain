...
import { ProxyChannel, getNextTickChannel } from 'bg/base/parts/ipc/common/ipc';
import { Client } from 'bg/base/parts/ipc/node/ipc.cp';
...
import { IWatcherRequest, IWatcherOptions, IWatcherService } from 'bg/platform/files/node/watcher/unix/watcher';
	...
	private startWatching(): void {
		const client = this._register(new Client(
			FileAccess.asFileUri('bootstrap-fork', require).fsPath,
			{
				...
				env: {
					BIGGAIN_AMD_ENTRYPOINT: 'bg/platform/files/node/watcher/unix/watcherApp',
					BIGGAIN_PIPE_LOGGING: 'true',
					BIGGAIN_VERBOSE_LOGGING: 'true' // transmit console logs from server to client
				}
			}
		));
