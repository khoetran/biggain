...
import { isMacintosh, isLinux, isWeb, isNative } from 'bg/base/common/platform';
...
import { LogLevel } from 'bg/platform/log/common/log';
...
export function getTitleBarStyle(configurationService: IConfigurationService): 'native' | 'custom' {
	...
	if (configuration) {
		...
		if (useSimpleFullScreen) {
			return 'native'; // simple fullscreen does not work well with custom title style
		}
