...
import { IConfigurationService } from 'bg/platform/configuration/common/configuration';
...
import { WindowMinimumSize, IWindowSettings, MenuBarVisibility, getTitleBarStyle, getMenuBarVisibility, zoomLevelToZoomFactor, INativeWindowConfiguration } from 'bg/platform/windows/common/windows';
import { Disposable } from 'bg/base/common/lifecycle';
import { browserCodeLoadingCacheStrategy, isLinux, isMacintosh, isWindows } from 'bg/base/common/platform';
import { defaultWindowState, ICodeWindow, ILoadEvent, IWindowState, LoadReason, WindowError, WindowMode } from 'bg/platform/windows/electron-main/windows';
import { ISerializableCommandAction } from 'bg/platform/actions/common/actions';
import { IThemeMainService } from 'bg/platform/theme/electron-main/themeMainService';
import { IDialogMainService } from 'bg/platform/dialogs/electron-main/dialogMainService';
import { mnemonicButtonLabel } from 'bg/base/common/labels';
import { ThemeIcon } from 'bg/platform/theme/common/themeService';
import { ILifecycleMainService } from 'bg/platform/lifecycle/electron-main/lifecycleMainService';
import { IStorageMainService } from 'bg/platform/storage/electron-main/storageMainService';
import { FileAccess, Schemas } from 'bg/base/common/network';
import { isLaunchedFromCli } from 'bg/platform/environment/node/argvHelper';
...
export class CodeWindow extends Disposable implements ICodeWindow {
	...
	constructor(
		...
		//#region create browser window
		{
			...
			const options: BrowserWindowConstructorOptions = {
				...
				webPreferences: {
					preload: FileAccess.asFileUri('bg/base/parts/sandbox/electron-browser/preload.js', require).fsPath,
					additionalArguments: [`--biggain-window-config=${this.configObjectUrl.resource.toString()}`],
			...
			if (browserCodeLoadingCacheStrategy) {
				this.logService.info(`window: using biggain-file:// protocol and V8 cache options: ${browserCodeLoadingCacheStrategy}`);
			} else {
				this.logService.info(`window: biggain-file:// protocol is explicitly disabled`);
			}
			...
			// a window on its own. In that case calling setBounds() would cause
	...
	private registerListeners(): void {
		...
		const supportedSvgSchemes = new Set([Schemas.file, Schemas.biggainFileResource, Schemas.biggainRemoteResource, 'devtools']); // TODO@mjbvz: handle webview origin
		...
		const isSafeFrame = (requestFrame: WebFrameMain | undefined): boolean => {
			for (let frame: WebFrameMain | null | undefined = requestFrame; frame; frame = frame.parent) {
				if (frame.url.startsWith(`${Schemas.biggainWebview}://`)) {
					return true;
				}
			}
			return false;
		...
		this._win.webContents.session.webRequest.onBeforeRequest((details, callback) => {
			...
			if (uri.path.endsWith('.svg')) {
				const isSafeResourceUrl = supportedSvgSchemes.has(uri.scheme) || uri.path.includes(Schemas.biggainRemoteResource);
		...
		this._win.webContents.session.webRequest.onHeadersReceived((details, callback) => {
			...
			if (contentTypes && Array.isArray(contentTypes)) {
				...
				// http://127.0.0.1:<port>/biggain-remote-resource?path=
				if (!uri.path.includes(Schemas.biggainRemoteResource) && contentTypes.some(contentType => contentType.toLowerCase().includes('image/svg'))) {
					return callback({ cancel: !isRequestFromSafeContext(details) });
				}
			}
		...
		this._win.on('enter-full-screen', () => {
			this.sendWhenReady('biggain:enterFullScreen', CancellationToken.None);
		});

		this._win.on('leave-full-screen', () => {
			this.sendWhenReady('biggain:leaveFullScreen', CancellationToken.None);
		});
			...
			case WindowError.CRASHED:
				...
				if (type === WindowError.UNRESPONSIVE) {
					...
					const result = await this.dialogMainService.showMessageBox({
						...
						type: 'warning',
						buttons: [
							mnemonicButtonLabel("&&Reopen"),
							mnemonicButtonLabel("&&Keep Waiting"),
							mnemonicButtonLabel("&&Close")
						],
				...
				// Crashed
					...
					if (!details) {
						...
						message = `The window has crashed (reason: '${details.reason}', code: '${details.exitCode ?? '<unknown>'}')`;
					}
					...
					const result = await this.dialogMainService.showMessageBox({
						...
						message,
						detail: "We are sorry for the inconvenience. You can reopen the window to continue where you left off.",
						noLink: true,
						defaultId: 0
					}, this._win);
	...
	load(configuration: INativeWindowConfiguration, options: ILoadOptions = Object.create(null)): void {
		...
		this._win.loadURL(FileAccess.asBrowserUri(this.environmentMainService.sandbox ?
			'bg/code/electron-sandbox/workbench/workbench.html' :
			'bg/code/electron-browser/workbench/workbench.html', require
		).toString(true));
	...
	private updateConfiguration(configuration: INativeWindowConfiguration, options: ILoadOptions): void {
		...
		// (as indicated by BIGGAIN_CLI environment), make sure to
	...
	serializeWindowState(): IWindowState {
		...
		if (this.isFullScreen) {
			...
			try {
				...
				// Electron has weird conditions under which it throws errors when
	...
	private getWorkingArea(display: Display): Rectangle | undefined {
		...
		// desktop being positioned somewhere.
		//
	...
	private setFullScreen(fullscreen: boolean): void {
		...
		// Events
		this.sendWhenReady(fullscreen ? 'biggain:enterFullScreen' : 'biggain:leaveFullScreen', CancellationToken.None);
	...
	private setMenuBarVisibility(visibility: MenuBarVisibility, notify: boolean = true): void {
		...
		if (visibility === 'toggle') {
			if (notify) {
				this.send('biggain:showInfoMessage', "You can still access the menu bar by pressing the Alt-key.");
			}
		}

		if (visibility === 'hidden') {
			// for some weird reason that I have no explanation for, the menu bar is not hiding when calling
			// this without timeout. there seems to be
	...
	private createTouchBarGroup(items: ISerializableCommandAction[] = []): TouchBarSegmentedControl {
		...
		const control = new TouchBar.TouchBarSegmentedControl({
			...
			change: (selectedIndex) => {
				this.sendWhenReady('biggain:runAction', CancellationToken.None, { id: (control.segments[selectedIndex] as ITouchBarSegment).id, from: 'touchbar' });
			}
		});
