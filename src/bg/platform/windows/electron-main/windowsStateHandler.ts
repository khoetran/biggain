...
import { INativeWindowConfiguration, IWindowSettings } from 'bg/platform/windows/common/windows';
import { defaultWindowState, ICodeWindow, IWindowsMainService, IWindowState as IWindowUIState, WindowMode } from 'bg/platform/windows/electron-main/windows';
	...
	getNewWindowState(configuration: INativeWindowConfiguration): INewWindowState {
		...
		// Window state is from a previous session: only allow fullscreen when we got updated or user wants to restore
			...
			if (allowFullscreen && isMacintosh && this.windowsMainService.getWindows().some(window => window.isFullScreen)) {
				...
				// state, we cannot allow more fullscreen windows.
		...
		if (windowConfig?.newWindowDimensions) {
			if (windowConfig.newWindowDimensions === 'maximized') {
				...
				if (lastActiveState.mode === WindowMode.Fullscreen) {
					state.mode = WindowMode.Fullscreen; // only take mode
				} else {
