...
import { StorageScope, IS_NEW_KEY, AbstractStorageService, StorageTarget } from 'bg/platform/storage/common/storage';
import { IStorage, Storage, IStorageDatabase, IUpdateRequest, InMemoryStorageDatabase } from 'bg/base/parts/storage/common/storage';
import { Promises } from 'bg/base/common/async';
import { ILogService } from 'bg/platform/log/common/log';
import { toErrorMessage } from 'bg/base/common/errorMessage';
...
export class BrowserStorageService extends AbstractStorageService {
	...
	private getId(scope: StorageScope): string {
		return scope === StorageScope.GLOBAL ? 'global' : 'local';
	}
	...
	protected async doInitialize(): Promise<void> {
		...
		this._register(this.workspaceStorage.onDidChangeStorage((key: string) => this.emitDidChangeValue(StorageScope.WORKSPACE, key)));
		...
		this._register(this.globalStorage.onDidChangeStorage((key: string) => this.emitDidChangeValue(StorageScope.GLOBAL, key)));
...
export class IndexedDBStorageDatabase extends Disposable implements IIndexedDBStorageDatabase {
	...
	private static readonly STORAGE_DATABASE_PREFIX = 'biggain-web-state-db-';
