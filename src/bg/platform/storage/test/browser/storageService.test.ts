...
import { BrowserStorageService, IndexedDBStorageDatabase } from 'bg/platform/storage/browser/storageService';
import { NullLogService } from 'bg/platform/log/common/log';
import { Storage } from 'bg/base/parts/storage/common/storage';
import { DisposableStore } from 'bg/base/common/lifecycle';
import { createSuite } from 'bg/platform/storage/test/common/storageService.test';
import { flakySuite } from 'bg/base/test/common/testUtils';
import { FileService } from 'bg/platform/files/common/fileService';
import { InMemoryFileSystemProvider } from 'bg/platform/files/common/inMemoryFilesystemProvider';
...
import { StorageScope, StorageTarget } from 'bg/platform/storage/common/storage';
...
async function createStorageService(): Promise<[DisposableStore, BrowserStorageService]> {
	...
	const storageService = disposables.add(new BrowserStorageService(logService, { userRoamingDataHome: URI.file('/User').with({ scheme: Schemas.userData }) } as unknown as IEnvironmentService, fileService));
