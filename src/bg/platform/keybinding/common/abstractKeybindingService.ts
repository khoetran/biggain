...
import { IntervalTimer, TimeoutTimer } from 'bg/base/common/async';
import { Emitter, Event } from 'bg/base/common/event';
import { KeyCode, Keybinding, ResolvedKeybinding } from 'bg/base/common/keyCodes';
...
import { IKeybindingEvent, IKeybindingService, IKeyboardEvent, KeybindingsSchemaContribution } from 'bg/platform/keybinding/common/keybinding';
import { IResolveResult, KeybindingResolver } from 'bg/platform/keybinding/common/keybindingResolver';
...
export abstract class AbstractKeybindingService extends Disposable implements IKeybindingService {
	...
	private _enterChordMode(firstPart: string, keypressLabel: string | null): void {
		...
		this._currentChordStatusMessage = this._notificationService.status(`(${keypressLabel}) was pressed. Waiting for second key of chord...`);
	...
	private _doDispatch(keybinding: ResolvedKeybinding, target: IContextKeyServiceTarget, isSingleModiferChord = false): boolean {
		...
		if (this._currentChord) {
			if (!resolveResult || !resolveResult.commandId) {
				this._notificationService.status(`The key combination (${this._currentChord.label}, ${keypressLabel}) is not a command.`, { hideAfter: 10 * 1000 /* 10s */ });
				shouldPreventDefault = true;
			}
		}
