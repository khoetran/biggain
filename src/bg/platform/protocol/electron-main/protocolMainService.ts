...
import { INativeEnvironmentService } from 'bg/platform/environment/common/environment';
import { ipcMain, session } from 'electron';
import { ILogService } from 'bg/platform/log/common/log';
import { TernarySearchTree } from 'bg/base/common/map';
import { isLinux, isPreferringBrowserCodeLoad } from 'bg/base/common/platform';
import { extname } from 'bg/base/common/resources';
import { IIPCObjectUrl, IProtocolMainService } from 'bg/platform/protocol/electron-main/protocol';
...
export class ProtocolMainService extends Disposable implements IProtocolMainService {
	...
	constructor(
		...
		// - storage    : all files in global and workspace storage
	...
	private handleProtocols(): void {
		...
		// Register biggain-file:// handler
		defaultSession.protocol.registerFileProtocol(Schemas.biggainFileResource, (request, callback) => this.handleResourceRequest(request, callback));
		...
		this._register(toDisposable(() => {
			defaultSession.protocol.unregisterProtocol(Schemas.biggainFileResource);
	...
	private handleResourceRequest(request: Electron.ProtocolRequest, callback: ProtocolCallback): void {
		...
		// Restore the `biggain-file` URI to a `file` URI so that we can
		...
		// finally block to load the resource
		this.logService.error(`${Schemas.biggainFileResource}: Refused to load resource ${fileUri.fsPath} from ${Schemas.biggainFileResource}: protocol (original URL: ${request.url})`);
	...
	createIPCObjectUrl<T>(): IIPCObjectUrl<T> {
		...
		const resource = URI.from({
			scheme: 'biggain', // used for all our IPC communication (biggain:<channel>)
			path: generateUuid()
		});
