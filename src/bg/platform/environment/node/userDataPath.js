...
(function () {
	...
	function factory(path, os, productName, cwd) {
		...
		function doGetUserDataPath(cliArgs) {
			...
			const portablePath = process.env['BIGGAIN_PORTABLE'];
			if (portablePath) {
			...
			// 3. Support global BIGGAIN_APPDATA environment variable
			let appDataPath = process.env['BIGGAIN_APPDATA'];
	...
	if (typeof define === 'function') {
		define(['require', 'path', 'os', 'bg/base/common/network', 'bg/base/common/resources', 'bg/base/common/process'], function (
			require,
		...
		module.exports = factory(path, os, pkg.name, process.env['BIGGAIN_CWD'] || process.cwd()); // commonjs
	} else {
