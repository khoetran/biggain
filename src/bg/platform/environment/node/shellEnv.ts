...
import { IProcessEnvironment, isWindows, OS } from 'bg/base/common/platform';
...
import { getSystemShell } from 'bg/base/node/shell';
import { CancellationTokenSource, CancellationToken } from 'bg/base/common/cancellation';
import { canceled, isPromiseCanceledError } from 'bg/base/common/errors';
...
export async function resolveShellEnv(logService: ILogService, args: NativeParsedArgs, env: IProcessEnvironment): Promise<typeof process.env> {
	...
	// Skip if running from CLI already
		...
		logService.trace('resolveShellEnv(): skipped (BIGGAIN_CLI is set)');
