...
if (!argv.debug) {
	app.setPath('userData', join(tmpdir(), `biggain-tests-${Date.now()}`));
}
...
app.on('ready', () => {
	...
	ipcMain.handle('biggain:test-biggain-window-config', async () => {
		return {
			product: {
				version: '1.x.y',
				nameShort: 'biggain',
	...
	ipcMain.handle('biggain:fetchShellEnv', event => { });
	...
	const win = new BrowserWindow({
		...
		show: false,
		webPreferences: {
			preload: path.join(__dirname, '..', '..', '..', 'src', 'bg', 'base', 'parts', 'sandbox', 'electron-browser', 'preload.js'), // ensure similar environment as biggain as tests may depend on this
			additionalArguments: [`--biggain-window-config=biggain:test-biggain-window-config`],
